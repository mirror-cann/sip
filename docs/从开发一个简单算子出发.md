...
本教程以能跑起来为第一目标，先不追求性能极致，力求让第一次接触SiP的开发者 30 min 内能在本地看到结果。


## 算子开发实例
我们以conj算子为例，说明基于SiP库开发一个算子的主要流程。
### 算子功能
...
```
### conj_kernel.cpp
文件路径：`ps/base/conj/conj/conj_kernel.cpp`  
主要功能：执行tiling，入参校验等。
```c++
...
### SiP编译与环境变量设置
SiP仓的构建脚本文件为`scripts/build.sh`，脚本使用的基本命令是：
```shell
bash build.sh
```
