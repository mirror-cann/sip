# 信号处理加速库AsdMulOperation C++ Demo

## 介绍

该目录下为信号处理加速库AsdMulOperation C++调用示例。

## 功能说明

- 算子功能：支持向量逐元素乘积(Hadamard)能力，返回一个和输入同样形状大小的复数矩阵。
- 计算公式：
    $$\text{z} = x \circ y = (x)_{ij}(y)_{ij}$$
    即两个矩阵对应位置的元素相乘。
...
- **example_mul_complex64.cpp**

    【注】：将编译脚本中的 example_mul_complex32.cpp 替换为 example_mul_complex64.cpp 后，替换后的编译脚本可编译运行。
