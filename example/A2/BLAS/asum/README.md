# 信号处理加速库AsumOperation C++ Demo

## 介绍

该目录下为信号处理加速库AsumOperation C++调用示例。

## 功能说明

- 算子功能：对输入的所有元素取绝对值后求和。
- 计算公式：
    - Sasum（单精度绝对值求和）的计算公式
        $$\text{result} = \sum_{i=0}^{n-1} |x_i|$$
    - Scasum（复数绝对值求和）的计算公式
...
- **example_scasum.cpp**
    ...
    【注】：将编译脚本中的 example_sasum.cpp 替换为 example_scasum.cpp 后，替换后的编译脚本可编译运行。
