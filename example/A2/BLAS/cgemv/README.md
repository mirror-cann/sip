# 信号处理加速库CgemvOperation C++ Demo
## 介绍
该目录下为信号处理加速库CgemvOperation C++调用示例。

## 功能说明

- 算子功能：一个矩阵向量乘法，用于计算复数矩阵A与复数向量x的乘积，结果存储在复数向量y中。
- 计算公式：
    $$\mathbf{y} = \alpha \cdot op(A) \cdot \mathbf{x} + \beta \cdot \mathbf{y}$$
    其中：
    - $A$ 为复数矩阵
