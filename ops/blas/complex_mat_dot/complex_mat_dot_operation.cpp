...
namespace AsdSip {
class ComplexMatDotOperation : public OperationBase {
public:
    explicit ComplexMatDotOperation(const std::string &opName) noexcept : OperationBase(opName) {}
    Kernel *GetBestKernel(const LaunchParam &launchParam) const override
    {
        ASDSIP_CHECK(IsConsistent(launchParam), "Failed to check consistent", return nullptr);
        return GetKernelByName("ComplexMatDotC64Kernel");
    }

    int64_t GetInputNum(const Any &specificParam) const override
    {
        ASDSIP_CHECK(specificParam.Type() == typeid(OpParam::ComplexMatDot), "OpParam is invalid", return 0);
        return INPUT_NUM;
    }
...
protected:
    Status InferShapeImpl(const LaunchParam &launchParam, SVector<Tensor> &outTensors) const override
    {
        ASDSIP_CHECK(launchParam.GetParam().Type() == typeid(OpParam::ComplexMatDot),
                "ComplexMatDot OpParam is invalid", return Status::FailStatus(ERROR_INVALID_VALUE));
...
REG_OPERATION(ComplexMatDotOperation);
}  // namespace AsdSip