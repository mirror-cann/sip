...
#include "log/log.h"
#include "dft.h"
#include "dft_sep_tiling_data.h"
#include "dft_sep_tiling.h"
...
constexpr uint32_t DFT_SIZE_MULTIPLIER = 1;
constexpr uint32_t WORKSPACE_SIZE = 32;

AsdSip::AspbStatus DftSepTiling(const LaunchParam &launchParam, KernelInfo &kernelInfo)
{
    const auto &param = AnyCast<OpParam::Dft>(launchParam.GetParam());

    DftSepTilingData *tilingDataPtr = reinterpret_cast<AsdSip::DftSepTilingData *>(kernelInfo.GetTilingHostAddr());
    ASDSIP_CHECK(tilingDataPtr != nullptr, "tilingDataPtr should not be empty",
              return AsdSip::ErrorType::ACL_ERROR_INVALID_PARAM);

    const uint32_t thresholdK = 32768;
    const uint32_t cubeDataCountPerLoopSmall = 128;
    const uint32_t cubeDataCountPerLoopLarge = 64;
