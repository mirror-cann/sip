...
AsdSip::AspbStatus FftBTiling(const LaunchParam &launchParam, KernelInfo &kernelInfo)
{
    const auto &param = AnyCast<OpParam::FftB>(launchParam.GetParam());

    FftBTilingData *tilingDataPtr = reinterpret_cast<AsdSip::FftBTilingData *>(kernelInfo.GetTilingHostAddr());
