...
class FftBSepOperation : public OperationBase {
public:
    explicit FftBSepOperation(const std::string &opName) noexcept : OperationBase(opName) {}
    Kernel *GetBestKernel(const LaunchParam &launchParam) const override
    {
        ...
        if (Mki::PlatformInfo::Instance().GetPlatformType() == Mki::PlatformType::ASCEND_910B) {
            return GetKernelByName("FftBSepC64Kernel");
        }
        return nullptr;
    }
...
REG_OPERATION(FftBSepOperation);
}  //    namespace AsdSip