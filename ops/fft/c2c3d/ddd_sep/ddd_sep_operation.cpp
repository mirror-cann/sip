...
class DddSepOperation : public OperationBase {
public:
    explicit DddSepOperation(const std::string &opName) noexcept : OperationBase(opName) {}

    Kernel *GetBestKernel(const LaunchParam &launchParam) const override
    {
        ASDSIP_CHECK(IsConsistent(launchParam), "Failed to check consistent", return nullptr);
        return GetKernelByName("DddSepC64Kernel");
    }

protected:
    Status InferShapeImpl(const LaunchParam &launchParam, SVector<Tensor> &outTensors) const override
    {
        ASDSIP_CHECK(launchParam.GetParam().Type() == typeid(OpParam::Ddd), "OpParam is invalid",
    ...
    {
        ASDSIP_CHECK(specificParam.Type() == typeid(OpParam::Ddd), "OpParam is invalid", return 0);
        return INPUT_NUM;
    }
