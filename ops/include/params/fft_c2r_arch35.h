...
#ifndef ASDSIP_PARAMS_FFT_C2R_ARCH35_H
#define ASDSIP_PARAMS_FFT_C2R_ARCH35_H
...
/**
 * Parameter struct for FftC2R Arch35 (Ascend 950) operation.
 *
 * This is a streamlined parameter for the mixed-radix FFT C2R kernel
 * that only supports prime radices 2, 3, 5, 7.
 *
 * Fields:
 *   fftN         - Full signal length (the output length of C2R)
 *   batchSize    - Number of batched FFTs
 *   radixListLen - Number of radix decomposition stages (length of plan)
 *   isInverse    - 1 for C2R (inverse), 0 for R2C (forward)
 */
struct FftC2RArch35 {
    int64_t fftN;
    int64_t batchSize;
    ...
    bool operator==(const FftC2RArch35 &other) const
    {
