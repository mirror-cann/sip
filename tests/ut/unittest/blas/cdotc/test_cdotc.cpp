...
std::string GetCdotcOutputDirectory() {
...
TEST(TestBlasCdotc, TestCdotcCase0)
{
    std::string originPath = GetCdotcOutputDirectory() + "/tests/ut/unittest/blas/cdotc/cdotc_data";
    std::string destPath = GetCdotcOutputDirectory() + "/build/tests/ut/unittest/blas/cdotc";
    std::string mkdir_cmd = "mkdir -p " + ShellQuote(destPath);
    std::string copy_cmd = "cp -rf " + ShellQuote(originPath) + " " + ShellQuote(destPath);
    std::string chmod_cmd = "chmod -R 755 " + ShellQuote(destPath + "/cdotc_data/");
    ...
    for (size_t i = 0; i < context.inTensors.size(); i++) {
        ...
        if (SaveTensorToBin(context.inTensors[i], destPath + "/cdotc_data/" + filename)) {
    ...
    std::string gen_data_cmd = "cd " + ShellQuote(destPath + "/cdotc_data/") + " && python3 gen_data.py";
    ...
    blasStatus = asdBlasCdotc(handle, n, aclInputX, incx, aclInputY, incy, aclResult);
    ASSERT_EQ(blasStatus, AsdSip::ErrorType::ACL_SUCCESS);
    ...
    if (SaveOutTensorToBin(resultTensor, destPath + "/cdotc_data/" + outFilename)) {
    ...
    std::string cmp_data_cmd = "cd " + ShellQuote(destPath + "/cdotc_data/") + " && python3 compare_data.py";
