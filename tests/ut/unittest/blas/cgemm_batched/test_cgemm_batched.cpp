...
at::Tensor CgemmBatchedGolden(at::Tensor a, asdBlasOperation_t transA, at::Tensor b, asdBlasOperation_t transB)
{
    at::Tensor golden;
    at::Tensor inA = a;
    at::Tensor inB = b;
...
void TestCgemm(int64_t batch, int64_t m, int64_t k, int64_t n) {
    ...
    inTensorDescs.push_back({TENSOR_DTYPE_COMPLEX64, TENSOR_FORMAT_ND, {batch, m, k}});
    inTensorDescs.push_back({TENSOR_DTYPE_COMPLEX64, TENSOR_FORMAT_ND, {batch, k, n}});
    ...
    at::Tensor golden = CgemmBatchedGolden(atA, transA, atB, transB);
    ...
    AspbStatus blasStatus = asdBlasCgemmBatched(handle, transA, transB, m, n, k, alpha, aclA, lda, aclB, ldb, beta, aclC, ldc, batch);
    ...
    aclrtMemcpy(c.hostData, batch * m * n * sizeof(std::complex<float>),
                cDeviceAddr, batch * m * n * sizeof(std::complex<float>), ACL_MEMCPY_DEVICE_TO_HOST);
...
TEST(TestOpCgemmBatched, TestCgemmBatched0)
{
    TestCgemm(15, 7, 8, 9);
}

TEST(TestOpCgemmBatched, TestCgemmBatched1)
{
    TestCgemm(200, 11, 10, 9);
}
