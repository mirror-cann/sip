...
at::Tensor CgercGolden(at::Tensor x, at::Tensor y, at::Tensor A, std::complex<float> alpha)
{
    at::Tensor golden;
    auto atAlpha = torch::complex(torch::tensor(alpha.real()), torch::tensor(alpha.imag()));

    at::Tensor yConj = at::conj(y);

    golden = atAlpha * at::outer(x, yConj) + A;
    return golden;
}
...
TEST(TestOpCgerc, TestCgercF32)
{
    ...
    at::Tensor golden = CgercGolden(atX, atY, atA, alpha);
    ...
    AspbStatus blasStatus = asdBlasCgerc(handle, m, n, alpha, aclX, incx, aclY, incy, aclA, lda);
    ...
    void *aDeviceAddr = Mki::GetStorageAddr(aclA);
    aclrtMemcpy(A.hostData,
        m * n * sizeof(std::complex<float>),
        aDeviceAddr,
        m * n * sizeof(std::complex<float>),
        ACL_MEMCPY_DEVICE_TO_HOST);

    at::Tensor ret = at::from_blob(A.hostData, ToIntArrayRef(A.desc.dims), at::kComplexFloat);
...
TEST(TestOpCgerc, TestCgercF32LongDims)
{
    Status status;
    std::complex<float> alpha = std::complex<float>{2.0, 2.0};
    int64_t m = 128;
