...
if __name__ == "__main__":
    np_type, x = load_tensor(curr_dir + "/float_input_0.bin")
    _, y = load_tensor(curr_dir + "/float_input_1.bin")
    golden = np.dot(x, y)
    np.array(golden, dtype=np_type).tofile(f"{np_type}_golden_t_sdot_0.bin")
