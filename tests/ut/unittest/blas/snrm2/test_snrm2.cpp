...
at::Tensor Snrm2Golden(at::Tensor x)
{
    at::Tensor golden;
    golden = at::norm(x);
    return golden;
...
TEST(TestOpSnrm2, TestSnrm2F32)
{
    ...
    at::Tensor golden = Snrm2Golden(atX);
    ...
    AspbStatus blasStatus = asdBlasSnrm2(handle, n, aclX, incx, aclY);
    ...
    void *outputYDeviceAddr = Mki::GetStorageAddr(aclY);
    aclrtMemcpy(y.hostData, sizeof(float), outputYDeviceAddr, sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
...
TEST(TestOpSnrm2, TestSnrm2F32Unaligned)
{
    int64_t n = 653;
    int64_t incx = 1;
