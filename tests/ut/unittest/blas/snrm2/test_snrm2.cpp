...
std::string GetSnrm2OutputDirectory() {
...
TEST(TestBlasSnrm2, TestSnrm2Case0)
{
    std::string originPath = GetSnrm2OutputDirectory() + "/tests/ut/unittest/blas/snrm2/snrm2_data";
    std::string destPath = GetSnrm2OutputDirectory() + "/build/tests/ut/unittest/blas/snrm2";
    std::string mkdir_cmd = "mkdir -p " + ShellQuote(destPath);
    std::string copy_cmd = "cp -rf " + ShellQuote(originPath) + " " + ShellQuote(destPath);
    std::string chmod_cmd = "chmod -R 755 " + ShellQuote(destPath + "/snrm2_data/");
    ...
    for (size_t i = 0; i < context.inTensors.size(); i++) {
        ...
        if (SaveTensorToBin(context.inTensors[i], destPath + "/snrm2_data/" + filename)) {
    ...
    std::string gen_data_cmd = "cd " + ShellQuote(destPath + "/snrm2_data/") + " && python3 gen_data.py";
    ...
    blasStatus = asdBlasSnrm2(handle, n, aclInputX, incx, aclResult);
    ASSERT_EQ(blasStatus, AsdSip::ErrorType::ACL_SUCCESS);
    ...
    if (SaveOutTensorToBin(resultTensor, destPath + "/snrm2_data/" + outFilename)) {
    ...
    std::string cmp_data_cmd = "cd " + ShellQuote(destPath + "/snrm2_data/") + " && python3 compare_data.py";
