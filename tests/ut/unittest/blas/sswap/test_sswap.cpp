...
TEST(TestOpSswap, TestSswapF32)
{
    ...
    void * goldenX = malloc(n * sizeof(float));
    void * goldenY = malloc(n * sizeof(float));

    memcpy_s(goldenX, n * sizeof(float), y.hostData, n * sizeof(float));
    memcpy_s(goldenY, n * sizeof(float), x.hostData, n * sizeof(float));

    at::Tensor atGoldenX = at::from_blob(goldenX, ToIntArrayRef(x.desc.dims), at::kFloat);
    at::Tensor atGoldenY = at::from_blob(goldenY, ToIntArrayRef(y.desc.dims), at::kFloat);
    ...
    AspbStatus blasStatus = asdBlasSswap(handle, n, aclX, incx, aclY, incy);
    ...
    aclrtMemcpy(x.hostData, n * sizeof(float), inputXDeviceAddr, n * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    ...
    at::Tensor retX = at::from_blob(x.hostData, ToIntArrayRef(x.desc.dims), at::kFloat);
    at::Tensor retY = at::from_blob(y.hostData, ToIntArrayRef(y.desc.dims), at::kFloat);
...
TEST(TestOpSswap, TestSswapF32Unaligned)
{
    int64_t n = 163;
    int64_t incx = 1;
    int64_t incy = 1;
