...
at::Tensor SsyrGolden(asdBlasFillMode_t uplo, float alpha, at::Tensor x, at::Tensor A)
{
    at::Tensor golden;
    if (uplo == asdBlasFillMode_t::ASDBLAS_FILL_MODE_UPPER) {
        golden = at::triu(alpha * at::outer(x, x)) + A;
    } else {
        golden = at::tril(alpha * at::outer(x, x)) + A;
    }
    return golden;
}
...
TEST(TestOpSsyr, TestSsyrF32Upper)
{
    Status status;
    ...
    at::Tensor golden = SsyrGolden(uplo, alpha, atX, atA);
    ...
    AspbStatus blasStatus = asdBlasSsyr(handle, uplo, n, alpha, aclX, incx, aclA, lda);
    ...
    aclrtMemcpy(A.hostData, 4 * sizeof(float), inputADeviceAddr, 4 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
...
TEST(TestOpSsyr, TestSsyrF32Lower)
{
    Status status;
    SVector<TensorDesc> inTensorDescs;
    inTensorDescs.push_back({TENSOR_DTYPE_FLOAT, TENSOR_FORMAT_ND, {2}});
