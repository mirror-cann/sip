...
if __name__ == "__main__":
    ...
    y = load_tensor(curr_dir + "/float_input_1.bin")
    A = load_tensor(curr_dir + "/float_input_2.bin")
    n = int(np.sqrt(A.size))
    A = A.reshape(n, n)
    update = A + alpha * np.outer(x, y) + alpha * np.outer(y, x)
    golden = A.copy()
    if uplo == "lower":
    ...
    golden.astype(np.float32).tofile(f"float_golden_t_ssyr2_0.bin")
