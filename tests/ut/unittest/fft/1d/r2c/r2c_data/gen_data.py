...
def compute_fft_r2c_inverse_golden(input_data, nfft):
    golden = np.fft.ihfft(input_data, n=nfft, norm='forward')
    return golden.astype(np.complex64)
...
def generate_golden_data(batch, nfft, direction):
    ...
    if direction == 'forward':
        ...
        for b in range(batch):
            batch_input = input_data[b, :]
            golden_data[b, :] = compute_fft_r2c_inverse_golden(batch_input, nfft)
        golden_filename = curr_dir + "/complex64_golden_r2c_inv.bin"
